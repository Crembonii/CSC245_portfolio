...
def run_game():
    ...
    # brick stats
    rows = 4  # max of 4 rows
    coll = 5 # columns (no max, but maybe don't push it.)
    ...
    loseFont = pygame.font.SysFont("comicsansms", 50)
    ...
    while (keepGoing):
        ...
        if b1.up == False:
            ...
            my_win.blit(my_text, (20, height/2))

        if win:
            my_win.fill(pygame.color.Color("black"))
            my_text = font.render("You win! You got all " + str(score) + " bricks!", False, "green")
            my_win.blit(my_text, (20, height/2))
